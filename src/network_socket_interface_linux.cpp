...
namespace ldlidar
{
    ...
    bool UDPSocketInterfaceLinux::CreateSocket(NetCommDevTypeDef obj, const char* ip, const char* port)
    {
        ...
        {
            LOG_ERROR("UDP,input ip address or port number is null", "");
            return false;
        }
        ...
        if ((UDP_CLIENT == obj) || (UDP_SERVER == obj))
        {
            ...
            if (com_sockfd_ == -1)
            {
                LOG_DEBUG("UDP,fail to socket. %s", strerror(errno));
                return false;
            }
        }
        else
        {
            LOG_ERROR("UDP,input value <obj> is error", "");
            return false;
        }
        ...
        if (obj == UDP_SERVER)
        {
            ...
            {
                LOG_ERROR("UDP,fail to bind. %s", strerror(errno));
                if (com_sockfd_ != -1)
                {
                    close(com_sockfd_);
                    com_sockfd_ = -1;
            ...
            LOG_INFO_LITE("UDP,bind success..", "");
        }
    ...
    bool TCPSocketInterfaceLinux::CreateSocket(NetCommDevTypeDef obj, const char* ip, const char* port)
    {
        ...
        if ((ip == nullptr) || (port == nullptr))
        {
            LOG_ERROR("TCP,input ip address or port number is null", "");
            return false;
        }
        ...
        if ((TCP_CLIENT == obj) || (TCP_SERVER == obj))
        {
            ...
            if (com_sockfd_ == -1)
            {
                LOG_DEBUG("TCP,fail to socket. %s", strerror(errno));
                return false;
            }
        }
        else
        {
            LOG_ERROR("TCP,input value <obj> is error", "");
            return false;
        }
        ...
        if (TCP_SERVER == obj)
        {
            ...
            if (-1 == bind(com_sockfd_, (const struct sockaddr*)&net_addr, (socklen_t)addrlen))
            {
                LOG_ERROR("TCP,fail to bind. %s", strerror(errno));
                if (com_sockfd_ != -1)
                {
                    close(com_sockfd_);
                    com_sockfd_ = -1;
            ...
            LOG_INFO_LITE("TCP,bind success..", "");

            // sockfd in listen
            if (-1 == listen(com_sockfd_, 1))
            {
                LOG_ERROR("TCP,fail to listen. %s", strerror(errno));
                if (com_sockfd_ != -1)
                {
                    close(com_sockfd_);
                    com_sockfd_ = -1;
            ...
            LOG_INFO_LITE("TCP,wait client connect.", "");
            ...
            {
                LOG_ERROR("TCP,fail to accept. %s", strerror(errno));
                if (com_sockfd_ != -1)
                {
                    close(com_sockfd_);
                    com_sockfd_ = -1;
            ...
            LOG_INFO_LITE("TCP,connect_client_fd: %d", listend_client_sockfd_);
            LOG_INFO_LITE("TCP,client port: %d", ntohs(client_addr.sin_port));
            LOG_INFO_LITE("TCP,client ip: %s", inet_ntoa(client_addr.sin_addr));
        }
        else if (TCP_CLIENT == obj)
        {
            ...
            {
                LOG_ERROR("TCP,connect server error. %s", strerror(errno));
                if (com_sockfd_ != -1)
                {
                    close(com_sockfd_);
                    com_sockfd_ = -1;
            ...
            LOG_INFO_LITE("TCP,connect server success.", "");
        }
