...
namespace ldlidar
{
    ...
    Tofbf::Tofbf(int speed, LDType type)
    {
        ...
        case LDType::STL_27L:
            ...
            break;
        default:
            std::cout << "tofbf input ldlidar type error!" << std::endl;
        ...
        const std::vector<PointData>& tmp) const
    {
        ...
        for (auto n : tmp)
        {
            ...
            if (n.distance < 500)
            {
                if ((n.distance + 10 < last_data.distance && n.distance + 10 < next_data.distance) || (n.distance > last_data.distance + 10 && n.distance > next_data.distance + 10))
                {
                    if (n.intensity < 60)
                    {
                        n.intensity = 0;
                ...
                else if ((n.distance + 7 < last_data.distance && n.distance + 7 < next_data.distance) || (n.distance > last_data.distance + 7 && n.distance > next_data.distance + 7))
                {
                    if (n.intensity < 45)
                    {
                        n.intensity = 0;
                ...
                else if ((n.distance + 5 < last_data.distance && n.distance + 5 < next_data.distance) || (n.distance > last_data.distance + 5 && n.distance > next_data.distance + 5))
                {
                    if (n.intensity < 30)
                    {
                        n.intensity = 0;
            ...
            if (n.distance < 5000)
            {
                ...
                if ((n.distance + 30 < last_data.distance || n.distance > last_data.distance + 30) && (n.distance + 30 < next_data.distance || n.distance > next_data.distance + 30))
                {
